...
sirs_results_systems %>% 
   ...
   summarise(., Wild_Prevalence_Change = paste0(round(median(Change_Prev_Wild)*100,4), " (", round(quantile(Change_Prev_Wild,probs = 0.1)*100,4), "-",round(quantile(Change_Prev_Wild,probs = 0.9)*100,4),")"), Captive_Prevalence_Change = paste0(round(median(Change_Prev_Captive)*100,4), " (", round(quantile(Change_Prev_Captive,probs = 0.1)*100,4), "-",round(quantile(Change_Prev_Captive,probs = 0.9)*100,4),")"), 
             Wild_Prevalence_Proportional_Change = paste0(round(median(Proportional_Change_Prev_Wild)*100, 6), " (", round(quantile(Proportional_Change_Prev_Wild,probs = 0.1)*100,6), "-",round(quantile(Proportional_Change_Prev_Wild,probs = 0.9)*100,6),")"), Proportional_Captive_Prevalence_Change = paste0(round(median(Proportional_Change_Prev_Captive)*100,6), " (", round(quantile(Proportional_Change_Prev_Captive,probs = 0.1)*100,6), "-",round(quantile(Proportional_Change_Prev_Captive,probs = 0.9)*100,4),")"),
             Wild_Incidence_Change = paste0(round(median(Change_Cumulative_Wild)*100,6), " (", round(quantile(Change_Cumulative_Wild,probs = 0.1)*100,6), "-",round(quantile(Change_Cumulative_Wild,probs = 0.9)*100,6),")"), Captive_Incidence_Change = paste0(round(median(Change_Cumulative_Captive)*100,6), " (", round(quantile(Change_Cumulative_Captive,probs = 0.1)*100,6), "-",round(quantile(Change_Cumulative_Captive,probs = 0.9)*100,6),")"), 
             Wild_Persist_Change = paste0(round(binom.confint(sum(Change_Persist_Wild),nsamples,methods = "exact")$mean,4), " (", round(binom.confint(sum(Change_Persist_Wild),nsamples,methods = "exact", conf.level = 0.80)$lower,4), "-", round(binom.confint(sum(Change_Persist_Wild),nsamples,methods = "exact", conf.level = 0.80)$upper,4), ")"),
