...
bind_rows(lapply(sirs_results_contexts$params, as.data.frame.list)) %>% 
mutate(run_id = as.numeric(row.names(.))) %>% 
   select(., "run_id", "beta_aero_ww","beta_aero_cw","beta_aero_cc","beta_aero_hw","beta_aero_hc", "beta_dc_ww", "beta_dc_cw", "beta_dc_cc", "I_human")-> parameters
...
sirs_results_contexts %>%
  ...
  group_by(Context, run_id) %>% 
   ...
   mutate(NewInfect_wild = S_wild*((beta_aero_ww*I_wild)+(beta_dc_ww*I_wild)+(beta_aero_cw*I_captive)+(beta_dc_cw*I_captive)+(beta_aero_hw*I_human)),
          NewInfect_captive = (S_captive * ((beta_aero_cc * I_captive) + (beta_dc_cc *I_captive) +
                                               (beta_aero_cw * I_wild) + (beta_dc_cw * I_wild) +
                                               (beta_aero_hc * I_human)))) %>% 
   summarize(Wild = mean(I_wild), Captive = mean(I_captive), Infected = Wild + Captive, Persist_wild = last(I_wild) >=0.0001, Persist_captive = last(I_captive) >=0.0001, Persist = as.logical(Persist_wild + Persist_captive), Cumulative_wild_infections = sum(NewInfect_wild), Cumulative_captive_infections = sum(NewInfect_captive)) %>% 
...
merge(r0.FOI,Prev_Persist_df[,c("run_id","Infected", "Persist", "Cumulative_wild_infections", "Cumulative_captive_infections")], by = "run_id", all.x = TRUE) %>% 
...
df %>% 
  group_by(., Context) %>% 
  reframe(., R0.Median = round(quantile(r0,probs = c(0.5)),2), FOI.Median = round(log10(quantile(FOI,probs = c(0.5))),2), Prevalence.Median = round(quantile(Infected, probs = c(0.5)),2), Persistence.Median = round(sum(Persist)/nsamples,2), Cumulative_cases_wild = median(Cumulative_wild_infections), Cumulative_cases_captive = median(Cumulative_captive_infections)) -> median.results
