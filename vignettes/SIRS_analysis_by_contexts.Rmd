...
omega_cc_ranch_no_bait <- calc_contact_rate(nsamples = nsamples, type_contact = habitat, N_w = nWild)
...
df %>% 
  ...
  reframe(., R0.Median = round(quantile(r0,probs = c(0.5)),2), 
          ...
          FOI.Median = round((quantile(FOI*100,probs = c(0.5))),4), 
          FOI.80CI = paste0(round((quantile(FOI*100,probs = c(0.1))),4)," - ",round((quantile(FOI*100,probs = c(0.9))),4)), 
