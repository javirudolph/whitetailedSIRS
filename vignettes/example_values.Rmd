...
sigma_dc <- get_EE_param_vals("Direct Contact Probability")
```


Using the sets of parameters from expert elicitation we calculate probability of infection via aerosol and direct contact. These calculations are also informed by parameters from the literature, and details on this can be found in the `sir_model_description` document/article. The calc_nu_aero and calc_nu_dc functions listed below are included in the `whitetailedSIRS` package.


```{r}
# aerosol transmission between deer, using deer exhalation/inhalation rates (default)
nu_aero_deer_deer <- whitetailedSIRS::calc_nu_aero(C_nu = C_nu,
                                  t_contact = t_contact_deer_deer / 60,
                                  r = r_deer)

# aerosol transmission from humans to deer, modifying exhalation rates to that of a human
nu_aero_deer_human_rural <- whitetailedSIRS::calc_nu_aero(ER = rep(0.53, nsamples), C_nu = C_nu, t_contact = t_contact_deer_human_rural/60, r = r_deer)
nu_aero_deer_human_capt <- whitetailedSIRS::calc_nu_aero(ER = rep(0.53, nsamples), C_nu = C_nu, t_contact = t_contact_deer_human_capt/60, r = r_deer)

# direct contact fluid transmission between deer
nu_dc_deer_deer <- whitetailedSIRS::calc_nu_dc(C_nu = C_nu)
...
list_params <- alternative(alpha_immunity = alpha_immunity, c_ww = c_ww, c_cw = c_cw, c_cc = c_cc, c_hw = c_hw, c_hc = c_hc, nu_aero_deer_deer_wild = nu_aero_deer_deer, nu_aero_deer_deer_captive = nu_aero_deer_deer, nu_aero_deer_human_wild = nu_aero_deer_human_rural, nu_aero_deer_human_capt = nu_aero_deer_human_capt, sigma_dc = sigma_dc, nu_dc_deer_deer = nu_dc_deer_deer, gamma_recov = gamma_recov, I_human = I_human, boost = boost)
...
```{r}
r0_deer_wild <- ((c_ww * nu_aero_deer_deer) + (c_ww * nu_dc_deer_deer * sigma_dc)) * 1/gamma_recov
r0_deer_captive <- ((c_cc * nu_aero_deer_deer) + (c_cc * nu_dc_deer_deer * sigma_dc)) * 1/gamma_recov
...
```{r}
#FOI
foi_wild <- c_hw * nu_aero_deer_human_rural * I_human
foi_captive <- c_hc * nu_aero_deer_human_capt * I_human
