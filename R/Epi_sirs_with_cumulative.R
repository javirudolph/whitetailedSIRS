...
#'
#' `Epi_sirs_with_cumulative` is written to work alongside the `deSolve::ode()` function, and
...
#'
#' deSolve::ode(y = example_inits, times = example_times, parms = example_params, func = whitetailedSIRS::Epi_sirs_with_cumulative)
#'
#'
Epi_sirs_with_cumulative <- function(time, state, parameters){
   with(as.list(c(state, parameters)), {
      ...
      return(list(c(dS_wild, dI_wild, dR_wild, cumulative_I_wild, dS_captive, dI_captive, dR_captive, cumulative_I_captive)))
   })
}
